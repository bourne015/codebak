...
void copy_to_fb(unsigned char *, unsigned char *);
...
/*---------------------------------------------------------------*/
	...
	buf = malloc(buf_size);
	if (buf == NULL) {
		printf("failed to malloc buf in app\n");
	}
	memset(buf, 0, buf_size);
	int count_r = 0;
	unsigned char test[buf_size];
	while(1)
	{
		count_r = read(v4l2_fd, buf, buf_size);
		printf("read: %d\n", count_r);
		if (count_r == 0)
			continue;
		printf("scr_size:%d, buf_size:%d\n", screen_size, buf_size);
		memcpy(fbmem, buf, count_r);
		printf("debug: %p\n", buf);
	}
